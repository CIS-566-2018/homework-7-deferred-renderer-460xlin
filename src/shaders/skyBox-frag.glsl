...
//    outColor = vec3(ndc * 0.5 + 0.5, 1);
    ...
    if(angle < sunSize)
    {
        ...
        else
        {
            ...
            outColor = mix(sunBlur, cloudColor, heightField * 0.75 * angle / 30.0);
        }
    }
...
////    outColor = distortedSkyHue;
   // fragColor[2] = vec4(gl_FragCoord.x/u_Dimensions.x, gl_FragCoord.y/ u_Dimensions.y,0.0, 1.0);
    fragColor[2] = vec4(outColor, 1.0);
}
